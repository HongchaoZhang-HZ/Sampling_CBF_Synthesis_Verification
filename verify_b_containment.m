function sol = verify_b_containment(b_coeff, degree, dd, A, g)
...
c_beta0 = sdpvar(length(mon_e_t), 1);
beta0 = c_beta0'*mon_e_t;
c_beta1 = sdpvar(length(mon_e_t), 1);
beta1 = c_beta1'*mon_e_t;
c_omega = sdpvar(length(mon_e_t), 1);
omega = c_omega'*mon_e_t;
...
h = 1 - x'*x;
p = omega*h + beta1*b + 1;
% p = omega*h + beta1*b + beta0 + 1;
% p = jacobian(b, x)*f - eta*b - theta*jacobian(b, x)*g - 1;
% p = -p;
F = [sos(beta1), sos(omega), sos(-p), sos(h)];
% F = [sos(beta0), sos(beta1), sos(omega), sos(-p)];
% options.solver = 'gurobi';
options = sdpsettings('solver', 'mosek', 'verbose', 2);
sol = solvesos(F, [c_beta0, c_beta1, c_omega], options);
